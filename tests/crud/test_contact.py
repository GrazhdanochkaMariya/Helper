...
from src.old.crud import contact_crud
from src.old.models import TypeEnum


@pytest.mark.asyncio
async def test_get_contact_by_profile(
    add_contacts_to_db: tuple, prepare_database: AsyncGenerator
):
    """Tests get contact by profile"""
    contact = await contact_crud.get_contact_by_profile(
        prepare_database, linkedin_profile=add_contacts_to_db[0].linkedin_profile
    )
...
async def test_delete_contact_by_profile(
    ...
    contact_deleted = await contact_crud.get_contact_by_profile(
        prepare_database, linkedin_profile=add_contacts_to_db[0].linkedin_profile
    )
    contact_existing = await contact_crud.get_contact_by_profile(
        prepare_database, linkedin_profile=add_contacts_to_db[1].linkedin_profile
    )
...
async def test_get_contacts(
    prepare_database: AsyncGenerator, add_contacts_to_db: tuple
):
...
async def test_update_contact(
    ...
    await contact_crud.update_contact(
        db=prepare_database, new_data=new_data, contact_id=add_contacts_to_db[0].id
    )
...
async def test_create_contact(prepare_database: AsyncGenerator):
    ...
    new_contact = await contact_crud.create_contact(
        db=prepare_database, new_data=new_contact_data
    )
